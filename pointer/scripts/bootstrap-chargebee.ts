...
async function upsertItemFamily() {
	const existing = await tryRetrieve(() =>
		cb.itemFamily.retrieve(itemFamily.id),
	);
	if (existing) {
...
async function upsertItems() {
	...
	for (const spec of allItems) {
		...
		const { id, name, type, item_family_id, description, external_name } = spec;
	...
	for (const planId of Object.keys(itemEntitlements) as PlanId[]) {
		...
		try {
			...
			const isGrandfather =
				...
				(typeof e?.message === "string" && e.message.includes("grandfather"));
