...
describe("resolveUsageRange", () => {
  ...
  it("buckets the longer ranges daily", () => {
    ...
    expect(resolveUsageRange("30d", PERIOD, LIMITS, NOW).from.toISOString()).toBe(
      "2026-06-29T14:20:33.000Z",
    );
  });
  ...
  it("grants the whole credit allowance over the full period", () => {
    ...
    const { allowances } = resolveUsageRange("period", wholePeriod, LIMITS, NOW);
