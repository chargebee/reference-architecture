...
describe("model entitlement mapping", () => {
  it("loads every documented model tier", async () => {
    ...
    expect(tiers.basic).toContain("gpt-4o-mini");
    expect(tiers.advanced).toContain("gpt-4o");
    expect(tiers.premium).toContain("gpt-5");
    expect(tiers.enterprise).toContain("*");
  });

  it("enforces lower tiers and enterprise wildcards", async () => {
    await expect(isModelAllowed("basic", "gpt-5")).resolves.toBe(false);
    await expect(isModelAllowed("premium", "gpt-5")).resolves.toBe(true);
