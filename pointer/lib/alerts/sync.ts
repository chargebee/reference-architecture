...
export async function processAlertWebhook(
  ...
  if (!alert?.id || !status?.subscription_id) {
    console.log("[alert] alert_status_changed received but missing alert.id or subscription_id — skipping", {
      eventId: event.id,
      alertId: alert?.id,
  ...
  console.log(`[alert] Redis alarm state updated — key pointer:alarm:${status.subscription_id}:${alert.id} → ${isAlarm ? "in_alarm" : "deleted"}`);
