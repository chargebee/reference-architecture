...
import { enqueueEntitlementSync, type EntitlementSyncJob } from "./queue";
...
export type EntitlementSyncTrigger = "webhook" | "checkout" | "reconcile";
...
async function impactedSubscriptionIds(
...
export async function processEntitlementWebhook(
	...
	if (
		DIRECT_EVENTS.has(eventType) ||
		SUBSCRIPTION_REFRESH_EVENTS.has(eventType)
	) {
		const subscriptionId = directSubscriptionId(content);
		if (!subscriptionId) {
			throw new Error(`${eventType} did not include a subscription id`);
